 ...
 */
export async function scrapeCity(cityName: string): Promise<void> {
  ...
  if (result.success && result.permits.length > 0) {
    ...
    console.log(`✅ ${cityName} scrape complete: ${result.permits.length} permits`);
  } else {
    console.log(`⚠️  ${cityName} scrape failed: ${result.error || "No permits found"}`);
  }
}
 ...
 */
export async function scrapeAllCities(): Promise<void> {
  ...
  for (const cityConfig of enabledCities) {
    try {
      await scrapeCity(cityConfig.city);
    } catch (error) {
